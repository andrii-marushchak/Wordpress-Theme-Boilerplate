...
// Team & Mentors Grid
{
    $('.mentors-grid-wrap').each(function () {
        const listWrap = $(this)
        ...
        // Init
        const MentorsGrid = new List(listWrap.attr('id'), {
            valueNames: ['team-item'],
            page: perPage,
            pagination: {
                'left': 1,
                'right': 1,
        ...
        function filterGrid() {
            ...
            MentorsGrid.filter(function (item) {
                let match = true;
        ...
        MentorsGrid.on('filterComplete', function () {
            if (listWrap.find('.team-item').length > 0) {
